...
static ngx_int_t ngx_http_image_get_orientation(const unsigned char *data, unsigned int size);
...
static ngx_int_t 
ngx_http_image_get_orientation(const unsigned char *data, unsigned int size){
	ngx_int_t orientation = 0;
  	ExifData *exifData = exif_data_new_from_data(data,size);
  	if (exifData){
...
transparent:
	...
	ngx_int_t orientation = ngx_http_image_get_orientation(ctx->image, ctx->length);
	
	ngx_int_t adjust_angle = 0;
	// default orientation info
	if (6 == orientation) {
		adjust_angle =360 - 90;
	}
	else if (3 == orientation) {
		adjust_angle =360 - 180;
	}
	else if (8 == orientation) {
		adjust_angle =360 - 270;
	}
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "!!!!!!!, angle:\"%d\"", ctx->angle); //handle 输出日志
    if (ctx->angle) {
        src = dst;
